...
struct StockChartView: View {
    ...
    @StateObject var quoteVM: IdeaQuoteViewModel
...
//            Divider().opacity(chartVM.selectedXOpacity)
    ...
    private var exchangeCurrencyView: some View {
        HStack(spacing: 4) {
            if let exchange = quoteVM.idea.exchDisp {
                Text(exchange)
            }
    ...
    private var headerView: some View {
        HStack {
            VStack(alignment: .leading, spacing: 4) {
                Text(quoteVM.idea.symbol)
                    .font(.title.bold())

                if let shortName = quoteVM.idea.shortname {
                    Text(shortName)
                        .font(.subheadline.weight(.semibold))
                        .foregroundColor(Color(uiColor: .secondaryLabel))
                }
...
struct StockTickerView_Previews: PreviewProvider {
    ...
    static var tradingStubsQuoteVM: IdeaQuoteViewModel = {
       var mockAPI = MockStocksAPI()
        mockAPI.stubbedFetchQuotesCallback = {
            [Quote.stub(isTrading: true)]
        }
        return IdeaQuoteViewModel(idea: .stub, stocksAPI: mockAPI)
    }()
    
    static var closedStubsQuoteVM: IdeaQuoteViewModel = {
       var mockAPI = MockStocksAPI()
        mockAPI.stubbedFetchQuotesCallback = {
    ...
    static var loadingStubsQuoteVM: IdeaQuoteViewModel = {
       var mockAPI = MockStocksAPI()
        mockAPI.stubbedFetchQuotesCallback = {
    ...
    static var errorStubsQuoteVM: IdeaQuoteViewModel = {
       var mockAPI = MockStocksAPI()
        mockAPI.stubbedFetchQuotesCallback = {
    ...
    static var chartVM: ChartViewModel {
        ChartViewModel(idea: .stub, apiService: MockStocksAPI())
    }
    
    static var previews: some View {
        Group {
            StockChartView(chartVM: chartVM, quoteVM: tradingStubsQuoteVM)
                .previewDisplayName("Trading")
                .frame(height: 700)
            
            StockChartView(chartVM: chartVM, quoteVM: closedStubsQuoteVM)
                .previewDisplayName("Closed")
                .frame(height: 700)
            
            StockChartView(chartVM: chartVM, quoteVM: loadingStubsQuoteVM)
                .previewDisplayName("Loading Quote")
                .frame(height: 700)
            
            StockChartView(chartVM: chartVM, quoteVM: errorStubsQuoteVM)
